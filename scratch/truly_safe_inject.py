...
# Restore pristine index.html
subprocess.run(['git', 'checkout', 'HEAD', '--', 'index.html'])

with open('index.html', 'r', encoding='utf-8') as f:
    text = f.read()

# 1. Add html2canvas script ONLY before the LAST </body>
if 'html2canvas.min.js' not in text:
    parts = text.rsplit('</body>', 1)
    text = parts[0] + '    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>\n</body>' + parts[1]
...
# 3. Add Javascript at the end (ONLY before the LAST </body>)
script_html = '''
...
if 'captureDonationPoster' not in text:
    parts = text.rsplit('</body>', 1)
    text = parts[0] + script_html + '\n</body>' + parts[1]

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("SAFELY injected scripts using rsplit!")
